...
try:
    ...
    class BlockChain:
        ...
        def checkvalidity(block, prevblock):
            ...
            elif prevblock.calculatehash != block.prevhash:return False
            elif not BlockChain.verifyingproof(block.proofno, prevblock.proofno):return False
            elif block.timestamp <= prevblock.timestamp:return False
            return True
        ...
        def proofofwork(lastproof):#identifies no f(prev f; new proof)' such that hash(ff') contain 4 leading zeroes
            proofno = 0
            while BlockChain.verifyingproof(proofno, lastproof) is False:proofno += 1
            return proofno
        @staticmethod
        ...
        @property
        def latestblock(self):return self.chain[-1]
